with t1 as (
    ...
    select store_id, array_agg(status)::text[] as stats, array_agg(timestamp_utc)::timestamptz[] stamps
    from store_status
    group by store_id
), t5 as (
    select t3.*, t4.stats, t4.stamps 
    from t3 
    left join t4 
    on t3.store_id=t4.store_id
    group by t3.store_id, t3.timezone_str,
        t3.start_day0, 
        t3.start_day1, 
        t3.start_day2, 
        t3.start_day3,
